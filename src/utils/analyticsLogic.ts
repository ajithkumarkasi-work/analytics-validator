...
const computeAttributePresence = (occurrences: RawEvent[] | undefined, attribute: string) => {
  ...
  occurrences.forEach((event) => {
    const hasAttribute = Object.keys(event).some((key) => {
      return normalizeKey(key) === normalizedAttribute && event[key] !== '' && event[key] != null
    })
...
const findColumnIndex = (headers: string[], aliases: string[]): number => {
  ...
  return lowered.findIndex((header) => aliases.some((alias) => header.includes(alias.toLowerCase())))
}
...
const buildGapForEvent = (
  ...
  descriptors.forEach((descriptor) => {
    const { present, total } = computeAttributePresence(occurrences, descriptor.name)
    const optional = descriptor.status === 'O'

    if (present === 0) {
      if (!optional) {
        missing.push({ name: descriptor.name, optional: false, present, total })
      }
      return
    }
...
const resolveEventStartIndex = (headerRow: string[]): number => {
  const sampleValuesIdx = headerRow.findIndex((cell) => /sample values/i.test(cell))
  if (sampleValuesIdx !== -1) {
    return sampleValuesIdx + 1
  }

  const descriptionIdx = headerRow.findIndex((cell) => /description/i.test(cell))
  if (descriptionIdx !== -1) {
  ...
  if (appStartIdx === -1) {
    throw new Error('Could not infer event columns (no Sample Values or Description or APP_START).')
  }
...
const determinePlatformInclusion = (
  ...
  web: indexes.web > -1 && Boolean(row[indexes.web] && !/^na$/i.test(row[indexes.web]?.trim() ?? '')),
  mobile:
    indexes.mobile > -1 &&
    Boolean(row[indexes.mobile] && !/^na$/i.test(row[indexes.mobile]?.trim() ?? '')),
  roku: indexes.roku > -1 && Boolean(row[indexes.roku] && !/^na$/i.test(row[indexes.roku]?.trim() ?? ''))
})
...
const processDictionaryAttribute = ({
  ...
  const attribute = buildAttribute(attributeName, rawAttribute, flagUpper, category)
...
const populateDictionaryMaps = (
  ...
  for (let rowIndex = headerRowIndex + 1; rowIndex < rows.length; rowIndex += 1) {
    ...
    for (let columnIndex = eventStartIdx; columnIndex < row.length; columnIndex += 1) {
...
export const parseDictionary = (text: string): DictionaryParseResult => {
  ...
  const events = headerRow.slice(eventStartIdx).filter((value) => value?.trim())
  if (!events.length) {
  ...
  const indexes = {
    ...
    mobile: findColumnIndex(headerRow, ['mobile platforms', 'mobile', 'ios', 'android']),
    roku: findColumnIndex(headerRow, ['roku'])
  }

  populateDictionaryMaps(rows, headerRowIndex, eventStartIdx, events, maps, indexes)
...
const ACTION_CANDIDATES = ['Action Name', 'actionName', 'name', 'event', 'action']
const PLAYBACK_KEYS = ['playbackSessionId', 'playback_session_id', 'playbacksessionid']
...
export const parseRealTimeData = (text: string): RealTimeParseResult => {
  ...
  for (const candidate of ACTION_CANDIDATES) {
    eventIndex = headers.findIndex((header) => header.toLowerCase() === candidate.toLowerCase())
...
export const calculateLetterStats = (events: RawEvent[], attribute: string): 'Y' | 'N' | 'P' => {
  ...
  events.forEach((event) => {
    ...
    const hasAttribute = Object.keys(event).some((key) => {
      return normalizeKey(key) === normalizedAttr && event[key] !== '' && event[key] != null
    })
...
const groupAndSortCoverageRows = (rows: CoverageRow[]): CoverageRow[] => {
  ...
  const sortedPrefixes = Array.from(grouped.keys()).sort((a, b) => a.localeCompare(b))
  ...
  eventNames.forEach((eventName) => {
    ...
    realTimeEvents.forEach((event) => {
      Object.keys(event).forEach((key) => {
        ...
        if (value !== '' && value != null) {
          presentCounts.set(normalized, (presentCounts.get(normalized) ?? 0) + 1)
        }
      })
    })
    ...
    const coverage =
      dictAttrs.length > 0
        ? Math.round(
            ((dictAttrs.length - missing.length - partial.length) / dictAttrs.length) * 100
          )
        : 0
